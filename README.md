...
- **v1.2.0**：**修「说了领完、App 里还在」** —— 批量领取碎片里有失败时旧版只打了一句
  「逐个重试」的日志、**实际没有重试**，失败原因还被丢掉；领取循环又在领取**之前**就把条目
  记成已领，一次失败就永不重试 → 通知写「可取皆已领完」，碎片却还在。
  现在：批量失败**真的逐个重试**、只有确实领到的才记为已领（失败项下一轮再试，最多 3 次）、
  失败原因写进日志与通知；还有没领掉的时通知标题变成 **「⚠️ 极氪签到（未领净）」** 并逐条列出名称与原因。
